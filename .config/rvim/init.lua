...
vim.keymap.set({ "n", "x" }, "<leader>gY", function() Snacks.gitbrowse({ open = function(url) vim.fn.setreg("+", url) end, notify = false, }) end, { desc = "Git Browse (copy)" })
-- Grep
...
vim.pack.add({ gh("saghen/blink.nvim"), gh("rafamadriz/friendly-snippets") })
require("blink").setup({
  keymap = { preset = "enter" },
})
